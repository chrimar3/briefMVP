export const meta = {
  name: 'score-round-v2',
  description: 'Round N review of WALKTHROUGH.html: hash-frozen candidate, hard-failure audit (repo fidelity + rendered UX) with adversarial verification, four audience tasks, three scoring personas, stability verdict, bounded editor brief',
  phases: [
...
const audits = await pipeline(AUDITORS,
...
const tasks = await parallel(AUDIENCE.map(a => () => agent(
...
const judges = (await parallel(PERSONAS.map((p, i) => () => agent(
  ...
  { label: `r${ROUND}:judge:${i + 1}`, phase: 'Score', schema: SCORE, model: 'opus' })))).filter(Boolean)
const by = {}
