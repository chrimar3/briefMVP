...
8. **Citation tags are machine-checked.** Every claim line in sections 1–7 ends with at least one tag of the form `[<source_id> <location>]`, where `source_id` is copied exactly from the brief's `meta.sources` — e.g. `[transcript_kickoff 00:14:32]`, `[rfp_meltemi §6]`. A claim line with no tag, or whose tags name no source the brief cites, is orphan prose and fails the run. Multiple supporting sources render as multiple tags.
